          ...
          <TableBody>
            {orders.map((order) => (
              ...
              >
                ...
                <TableCell className="px-10">
                  <div className="flex items-center gap-3 text-[10px] font-bold text-zinc-500 uppercase tracking-tighter italic">
                    {order.employeeId ? (
                       <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-xl bg-zinc-900 border-2 border-white shadow-md text-white flex items-center justify-center font-black uppercase italic text-xs">
                           {employees.find(e => e.uid === order.employeeId)?.name?.charAt(0) || "A"}
                        </div>
                        <span className="font-black text-zinc-600">{employees.find(e => e.uid === order.employeeId)?.name || "Agent"}</span>
                       </div>
                    ) : ( 
                      <span className="text-orange-500 flex items-center gap-2 font-black italic bg-orange-50 px-3 py-1.5 rounded-full scale-95 group-hover:scale-100 transition-transform">
            ...
            {orders.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="h-64 text-center bg-zinc-50/20">
