...
export const SupplierNetwork: React.FC<SupplierNetworkProps> = ({
  ...
  const showRetailers = mode === "all" || mode === "retailers";
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {showEmployees && (
        <div className="space-y-6">
          ...
          {employees.length === 0 ? (
            <div className="bg-white rounded-[2rem] p-8 border border-zinc-100 flex flex-col items-center justify-center text-center">
              <Users className="h-12 w-12 text-zinc-300 mb-3" />
              <p className="text-sm font-bold text-zinc-500 uppercase tracking-wider">No active employees found</p>
              <p className="text-xs text-zinc-400 mt-1">Use the Provision link to invite employees to join your organization.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
                {employees.map(emp => (
                <Card key={`network-emp-${emp.uid}`} className="rounded-[2.5rem] border-none shadow-2xl bg-white overflow-hidden group hover:scale-[1.03] transition-all duration-500 cursor-pointer h-full flex flex-col" onClick={() => navigate(`/supplier/employees/${emp.uid}`)}>
      ...
      {showRetailers && (
        <div className="space-y-6">
          ...
          {retailers.length === 0 ? (
            <div className="bg-zinc-900 rounded-[2rem] p-8 text-white/90 border border-white/5 flex flex-col items-center justify-center text-center">
              <Store className="h-12 w-12 text-zinc-500 mb-3" />
              <p className="text-sm font-bold text-white/50 uppercase tracking-wider">No retail partners found</p>
              <p className="text-xs text-white/40 mt-1">Use the Provision link to invite retailer partners to register.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
                {retailers.map(ret => (
                <Card key={`network-ret-${ret.uid}`} className="rounded-[2.5rem] border-none shadow-2xl bg-zinc-900 text-white overflow-hidden group hover:scale-[1.03] transition-all duration-500 cursor-pointer h-full flex flex-col" onClick={() => navigate(`/supplier/retailers/${ret.uid}`)}>
                    <CardHeader className="p-8 pb-4">
                    <div className="flex items-start gap-6">
